...
[#11](https://github.com/freedomRemains/remainz-v2/issues/11)。

## スコープ

- マイページ(`GET/POST /remainz-v2/service/myPage.html`)の表示・ログイン処理。
...
- `SCR_ELM.txt` の `SCR_ELM_ID = 1100201, 1100202, 1100251, 1100252, 1100253` について、
  `SERVICE_NAME` を `com.remainz.web.service.web.*` から `com.freedom.remainz_v2.web.service.*`
...
`T(com.freedom.remainz_v2.web.util.AuthUtil).hasReadAuth('1000001', authList)` を用いる。これは
...
| スクリプトレットの`if` | `th:if` |
| `request.getAttribute(...)` | Model属性(`RemainzV2Controller`が`populateModel`等で設定済み) |
| `<%= account.get(0).get("ACCOUNT_NAME") %>` | `th:text="${account[0].ACCOUNT_NAME}"` |
| `AuthUtil.hasReadAuth(...)` | `T(com.freedom.remainz_v2.web.util.AuthUtil).hasReadAuth(...)` |
...
- `RemainzV2ControllerTest`に`myPage.html`の`@GetMapping`/`@PostMapping`に関するテストケースを
  追加する。
...
- `.github/copilot-instructions.md`に、今後の画面移植issue向けの一般的な移植手順を追記する:
  ...
  3. 必要なDBデータ(`SCR_ELM`のサービスクラス名等)を`com.freedom.remainz_v2.*`へ更新し、
