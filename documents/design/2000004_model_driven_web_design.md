...
`RemainzV2Controller` の仮実装を廃止し、DBレコード(`URI_PATTERN`/`HTML_PAGE`/`SCR`/`SCR_ELM`/`SCR_PRM`/
...
[#9](https://github.com/freedomRemains/remainz-v2/issues/9)。

## スコープ

- TOPページ(`GET /remainz-v2/service/top.html`)が、DBレコード駆動で表示できること。
...
- `RemainzV2Controller#getTop` を、移植元 `controllService` に相当する共通処理呼び出しに変更する。
