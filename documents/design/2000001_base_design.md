...
src/main/java/com/remainz
	...
	web                 Webアプリの資材を配置する
		...
		servlet             Webアプリのサーブレット資材を配置する(remainz-v2では廃止)
		util                Webアプリに固有のユーティリティ資材を配置する
```

- 移植先である「remainz-v2」のパッケージ構成は、次の通りとします。

```
src/main/java/com/freedom/remainz_v2
	common              Webやバッチなどの仕組みに依存しない、共通的な資材を配置する
	    db                  DBに関連する資材を配置する
	    exception           共通例外の資材を配置する
	    service             共通業務ロジック(サービス)の資材を配置する
...
- 仮置きの実装となっている「RemainzV2Controller」は上記パッケージ構成に従っていませんので、移動してください。
- なおコントローラは「RemainzV2Controller」のみとします。
...
- 「RemainzV2Controller」では、DBレコードに基づいて処理を行うサービスを呼び出すのみとします。
