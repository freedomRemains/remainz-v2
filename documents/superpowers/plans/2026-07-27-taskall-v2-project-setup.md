...
**Goal:** remainz-v2の資材をコピーし、命名・ドキュメントを調整した上で、GitHub organization
`freedomRemains`配下に完全新規・別管理の`taskall-v2`リポジトリとして立ち上げる。

**Architecture:** remainz-v2の作業ツリーを`.git`を除いてローカルコピーし、`remainz`系の命名を
`taskall`系へ一括置換した後、新規リポジトリとして初回コミット・pushする。既存remainz-v2は
一切変更しない(読み取り専用の参照元として扱う)。

**Tech Stack:** Java 21 / Spring Boot / Gradle / Thymeleaf / SQLite (remainz-v2と同一のまま)

## Global Constraints

- 参照元`/home/develop/remainz-v2`は一切変更しない(コピー元として読むだけ)。
- コピー先は`/home/develop/taskall-v2`とする。
- Javaパッケージは`com.freedom.remainz_v2` → `com.freedom.taskall_v2`にリネームする。
- Gradleプロジェクト名・アプリ名・URLマッピング・クラス名は`remainz-v2`/`remainz_v2`/`RemainzV2`
  → `taskall-v2`/`taskall_v2`/`TaskallV2`へ一括置換する。
- DBファイル名`taskallv2.db`は変更しない(既存remainz-v2側で既にこの名称のため)。
- 環境変数名`REMAINZ_DATASOURCE_URL` → `TASKALL_DATASOURCE_URL`に変更する。
- `documents/prompts/`, `documents/rules/`, `.superpowers/sdd/`配下の中身(remainz-v2の開発履歴)
...
- ブランチ運用はremainz-v2と同じ規則(`main` ← `develop` ← `feature/<issue番号>`)を踏襲する。
...
- Visibility: remainz-v2と同じ設定(Private/Publicはremainz-v2の現状設定に合わせる)
- 「Initialize this repository with」配下の README/.gitignore/license は**すべてチェックしない**
  (空リポジトリとして作成し、後続タスクでローカルの完全な資材一式をpushするため)。

- [ ] **Step 2: developブランチ保護ルールの設定(任意、remainz-v2と揃える場合)**

remainz-v2の`develop`ブランチにはPR必須のルールが設定されている
...
**Files:**
- Create: `/home/develop/taskall-v2/` (remainz-v2の作業ツリーのコピー)

**Interfaces:**
- Consumes: `/home/develop/remainz-v2`の現行作業ツリー(変更しない)
- Produces: `/home/develop/taskall-v2`配下に、`.git`・ビルド生成物・remainz-v2固有の
  開発履歴を除いた作業ツリー一式
...
rsync -a --exclude='.git/' --exclude='build/' --exclude='.gradle/' \
  ...
  /home/develop/remainz-v2/ /home/develop/taskall-v2/
```
...
remainz-v2の開発履歴(過去のAI回答)はtaskall-v2に引き継がない。フォルダ構成のみ残し、
...
Expected: 各フォルダに`README.md`のみ、もしくは0件(remainz-v2側にREADMEが無ければ後続タスクで
新規作成する)。
...
```bash
cd /home/develop/taskall-v2
sed -i "s/rootProject.name = 'remainz-v2'/rootProject.name = 'taskall-v2'/" settings.gradle
cat settings.gradle
```
...
```bash
cd /home/develop/taskall-v2
sed -i 's/name: remainz-v2/name: taskall-v2/' src/main/resources/application.yaml
...
- [ ] **Step 4: URLマッピング・GitHubリンクの一括置換(remainz-v2 → taskall-v2)**

`.java`, `.yaml`, `.md`, `.gradle`ファイルすべてを対象に、残っている`remainz-v2`表記を
`taskall-v2`に置換する(パッケージ名`remainz_v2`はTask 3で対応済みのため、ここでは
ハイフン区切りの`remainz-v2`のみが対象)。

```bash
cd /home/develop/taskall-v2
grep -rl 'remainz-v2' --include='*.java' --include='*.yaml' --include='*.md' --include='*.gradle' . \
  | xargs sed -i 's/remainz-v2/taskall-v2/g'
```
...
```bash
cd /home/develop/taskall-v2
grep -rn 'remainz-v2' --include='*.java' --include='*.yaml' --include='*.md' --include='*.gradle' .
```
...
remainz-v2の`.github/copilot-instructions.md`には、ローカルクローン`/home/develop/remainz`を
...
- Produces: 設計ドキュメントの内容が「remainz-v2エンジン自身の設計根拠」として妥当であることの
  確認結果(このタスクはレビューのみで、原則ファイル変更は行わない)

`documents/design/`配下の設計書には`com.remainz`(旧remainz v1)への言及が複数残るが、これらは
「なぜ現在のエンジン構造になっているか」という**remainz-v2自身の設計根拠・沿革**を説明する記述
...
出力された各行について、「remainz(v1)からremainz-v2への移植時にこうした」という
過去の経緯説明になっていること(＝taskall-v2への指示ではないこと)を確認する。
もし「今後もremainzを参照して移植すること」という**現在進行形の指示**になっている
記述が見つかった場合のみ、その一文を「remainz-v2で確立済みの設計のため、taskall-v2では
そのまま踏襲する」という趣旨に修正する。
...
`build/`, `.gradle/`等のビルド生成物がステージされていないことを、remainz-v2の
...
chore: remainz-v2からtaskall-v2プロジェクトを立ち上げ

remainz-v2の作業ツリーをコピーし、パッケージ名・アプリ名・環境変数名等を
taskall-v2向けにリネーム。移植元プロジェクトについてのドキュメントは
「助か～る」向けに書き換え、documents/prompts・documents/rules配下の
remainz-v2固有の開発履歴は含めていない。
...
- [ ] **Step 4: mainブランチも作成してpushする(remainz-v2と同じブランチ運用のため)**
