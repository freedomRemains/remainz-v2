package com.freedom.remainz_v2.web.aspect;
...
import com.freedom.remainz_v2.web.controller.RemainzV2Controller;
...
class ControllerLoggingAspectTest {
    ...
    @Autowired
    private RemainzV2Controller remainzV2Controller;
    ...
    void コントローラがAOPプロキシとして注入されていること() {
        assertThat(AopUtils.isAopProxy(remainzV2Controller)).isTrue();
    }
    ...
    void コントローラ実行後に処理時間がログ出力されること() throws Exception {
        ...
        mockMvc.perform(get("/remainz-v2/service/top.html")).andExpect(status().is3xxRedirection());
